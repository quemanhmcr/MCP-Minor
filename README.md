...
- `get_file_skeleton`: read-only AST-backed structural outline for JavaScript and TypeScript-family source files. Input: `paths: string[]`, optional `limit`. Supported extensions are `.js`, `.jsx`, `.mjs`, `.cjs`, `.ts`, and `.tsx`. Results include absolute `path`, stable `relativePath`, `language`, `rootType`, `sourceLength`, `hasParseErrors`, `entryCount`, `limit`, `truncated`, and nested `entries` with `kind`, `name`, `signature`, line/byte `location`, and `children`. It preserves imports, export-only statements, exported definitions, top-level functions/classes, class/interface methods, named arrow/function expressions, TypeScript interfaces/types/enums, duplicate names in different scopes, and practical nested members. Missing paths, directories, symlinks, out-of-workspace paths, unsupported extensions, rich/binary files, invalid UTF-8, binary-looking files, and files over the 1MB parse safety cap fail before parsing.
...
- `get_file_skeleton` intentionally returns structured JSON instead of Dirac's formatted text skeleton. It does not return edit anchors; line/byte locations are non-edit metadata. Use `read_file` when edit-compatible anchors are needed. It does not compute Dirac's optional call graph comments, does not integrate `.diracignore`, and currently supports only the JS/TS language family packaged in this server. Parser recovery is surfaced through `hasParseErrors` instead of hiding syntax problems.
