...
- `source` default: compact text with `relative/path::qualifiedName`, kind, line range, bounded signature, suffix-match request marker when relevant, parse-error marker only when relevant, truncation marker, and bounded line-numbered source. Structured content includes compact per-match metadata but omits source text.
...
Name matching trims whitespace, normalizes Unicode to NFC for comparison, treats `::` as `.`, prefers exact qualified names, then falls back to suffix-qualified names. Examples: `buildName`, `Greeter.getName`, and `getName`. Ambiguous suffix matches return all candidates plus fully qualified names by default; set `requireUnique: true` for strict ambiguity errors. Missing names are returned in metadata even when no functions match. TypeScript overload declarations are collapsed to the implementation when one is present; signature-only ambient overloads remain as multiple candidates.

Addressing conventions: anonymous default function exports are named `default`; private methods keep the hash, for example `Box.#secret`; computed method names use the bracket text emitted by tree-sitter, for example `[Symbol.iterator]`.

Range and hashes: attached single-line decorators immediately above a method are included in the returned function range even when `contextLines` is `0`. `bodyHash` is computed from the UTF-8 bytes of the returned function range, including decorators when present, with no whitespace normalization. `viewHash` is computed from the bounded returned source text without line-number prefixes, edit anchors, or truncation marker.

Notes: default output intentionally omits edit anchors and defaults `contextLines` to `0`. Requests are capped at 200 paths, 200 names, and 200 total returned match/missing/ambiguous items to bound parse and output cost without rejecting broad path lists solely by `paths.length * function_names.length`. `maxSourceChars` defaults to 30,000, has a 64 character minimum, and is capped at 120,000. Truncated compact text includes a `trunc:lines`, `trunc:chars`, or `trunc:lines+chars` header marker plus a source marker so text-only agents know which knob to retry. Use `view: "edit"` when the next step is mutation; its anchor block is byte-identical to `read_file view: "edit"` for the same returned line range. The tool uses the current tree-sitter skeleton scope, so it supports functions, named arrow/function expressions, object shorthand methods, constructors, getters/setters, generators, default exports, private methods, computed methods, and decorated methods already visible to `get_file_skeleton`; it does not use a symbol index and does not find references, call sites, or non-JS/TS symbols.

Safety note: `get_function` first resolves paths through workspace-root containment and symlink checks in `get_file_skeleton`, then revalidates containment, file type, realpath, size, binary-looking content, and UTF-8 before reading source for extraction. Like the read-only file tools, it does not hold an open file handle across validation and read, so a local filesystem TOCTOU swap between validation and `fs.readFile` remains in the v1 threat model.
